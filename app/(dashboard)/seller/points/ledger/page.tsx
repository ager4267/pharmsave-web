...
export default function UserLedgerPage() {
  ...
  const fetchLedger = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/user/ledger?${params.toString()}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="mt-2 text-gray-600">나의 입금 내역과 포인트 충전 내역을 확인합니다.</p>
        </div>

        {/* 필터 섹션 */}
        <div className="bg-white shadow rounded-lg p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">기간 필터</h2>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {deposits.map((deposit) => (
                    <tr key={deposit.id}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">
                        {deposit.admin ? deposit.admin.company_name : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {transactions.map((tx) => (
                    <tr key={tx.id}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">
                        {tx.admin ? tx.admin.company_name : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
